 ...
 *
 * Press Alt+P to send your current prompt in "plan mode" — the agent will
 * research and plan without making any changes.
 *
 * - Alt+P reads the editor text, clears the editor, and sends the prompt
 *   wrapped in plan-mode instructions.
 * - Write/edit tools are disabled for the plan turn.
 * - Destructive bash commands are blocked.
 * - A footer status indicator shows when plan mode is active.
 */
...
export default function (pi: ExtensionAPI): void {
	...
	function updateStatus(ctx: ExtensionContext): void {
		if (planTurnActive) {
			ctx.ui.setStatus("plan-prompt", ctx.ui.theme.fg("warning", "📋 plan"));
		} else {
			ctx.ui.setStatus("plan-prompt", undefined);
		}
	}

	// Alt+P: grab editor text, send as plan-mode prompt
	pi.registerShortcut("alt+p", {
		description: "Send prompt in plan mode (research & planning, no changes)",
		handler: async (ctx) => {
			const text = ctx.ui.getEditorText()?.trim();
			if (!text) {
				ctx.ui.notify("Editor is empty — type a prompt first, then press Alt+P.", "warning");
				return;
			}
			...
			const fullPrompt = PLAN_PROMPT.replace("$ARGUMENTS", text);
			pi.sendUserMessage(fullPrompt);
		},
	});

	// Clear status when the agent finishes
	pi.on("agent_end", async (_event, ctx) => {
		if (!planTurnActive) return;
